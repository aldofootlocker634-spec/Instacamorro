#!/usr/bin/env python3
"""
🌐 Instagram Profile Viewer - Professional Bot
...
class InstagramBotWeb:
    ...
    def get_profile_info(self, username=None):
        try:
            if username is None:
    ...
    def get_user_medias(self, username=None, amount=12):
        try:
            if username is None:
                username = self.bot.username
    ...
    def get_stories(self, username=None):
        try:
            if username is None:
                username = self.bot.username
    ...
    def like(self, media_id):
        try:
            ...
            self.logs.append(f"👍 تم اللايك على منشور {media_id}")
        except Exception as e:
    ...
    def comment(self, media_id, text):
        try:
            ...
            self.logs.append(f"💬 تم التعليق: {text}")
        except Exception as e:
    ...
    def run(self, target_username, wait_time):
        ...
        while self.is_running:
            try:
                ...
                if not new_posts:
                    self.logs.append(f"⏰ جاري الانتظار... ({wait_time//60} دقيقة)")
...
def login():
    ...
    if instagram_bot.login(username, password):
        ...
        return jsonify({"success": False, "message": "❌ فشل تسجيل الدخول - تحقق من البيانات"})

@app.route("/api/profile", methods=["GET"])
def get_profile():
    username = request.args.get("username")
    
    if instagram_bot.bot is None or not instagram_bot.bot.api.is_logged_in():
    ...
    if info:
        ...
        return jsonify({"success": False, "message": "❌ لم يتم العثور على الملف"})
...
def get_media():
    ...
    if medias:
        ...
        return jsonify({"success": False, "message": "❌ لم يتم العثور على منشورات"})
...
def start_bot():
    ...
    if not config:
        return jsonify({"success": False, "message": "❌ الرجاء تسجيل الدخول أولاً"})
    ...
    target = data.get("target", config.get("username"))
    ...
    return jsonify({"success": True, "message": f"🚀 تم بدء مراقبة @{target}"})

@app.route("/api/stop", methods=["POST"])
def stop_bot():
    instagram_bot.stop()
    return jsonify({"success": True, "message": "⏹️ تم إيقاف البوت"})
